...
def gen_target(ep):
    ...
    for a in ep[0][:turn_idx]:#試合が終わるまでループ
        state.play(a)#試合を行う
    v = ep[1]
    return state.feature(), ep[2][turn_idx], [v if turn_idx % 2 == 0 else -v]# ニューラルネットに入力する状態表現を返す
...
def train(episodes):#トレーニング用の関数
    ...
    for epoch in range(num_epochs):#ループ
        ...
        for i in range(0, len(episodes), batch_size):#ループ
    ...
    print('p_loss %f v_loss %f' % (p_loss_sum / len(episodes), v_loss_sum / len(episodes)))#p,vのlossをprintさせる
    return net
