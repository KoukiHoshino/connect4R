...
from State import State_6_8_4_train
from Net import Net
...
def AI_agent(state,net1,display):
    # ランダムな手を返すエージェント
    legal_actions = state.legal_actions()
    #net1 = Net()
    net1.load_state_dict(torch.load(f"models/sim0_UCB_400_50.pth"))
...
def human_agent(state,net,display):
    # ユーザーが手を指定するエージェント
    legal_actions = state.legal_actions()
    ...
    while True:
        ...
        action_str = display.put_stone(state)
        if len(action_str) != 2:
...
def play_game(player1, player2,display):
    state = State()
    net = Net()
    count111 = 0
    print(state)
    while not state.terminal():
        if state.color == BLACK:
            action = player1(state,net,display)
            if action == -1:
              ...
              state.play(action,net,state)
              count111 = 0
        else:
            action = player2(state,net,display)
            if action == -1:
              #state.color == BLACK
              #state.color = - state.color
              count111 = count111 + 1
    ...
    print("win_color: " + str(state.win_color))
    print(state)
...
if __name__ == "__main__":
  ...
  while(1):
    if display.start():
      ...
      play_game(human_agent, AI_agent, display)
      #result = play_game(human_agent, AI_agent, display)
